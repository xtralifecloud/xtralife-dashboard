...
process.on('uncaughtException', err => logger.fatal(err.message, {stack: err.stack}, err => process.exit(1)));
...
const _loadUser = function(username, cb){
	if (xlenv.dashboard.users[username] == null) { return cb(new Error('Incorrect username.')); }

	const ident = _.extend({}, xlenv.dashboard.users[username]);
	ident.name = username;
	ident.games = _getGames(ident.games);

	return cb(null, ident);
};
...
passport.deserializeUser((user, done) => _loadUser(user, (err, ident) => done(null, ident || null)));
