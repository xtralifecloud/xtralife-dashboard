...
route.param('game', function(req, res, next, game){
	for (let each of Array.from((req.user != null ? req.user.games : undefined))) { if ((each != null ? each.name : undefined) === req.params.game) { game = each; } }
	if (game != null) {
...
route.param('domain', function(req, res, next, domain){
	...
	if (_isValidDomain(req.params.domain)) {
		...
		.json({
			Error: 'Unauthorized',
			Message: "Access not granted"}).end();
	}
});
...
.get((req, res) => xtralife.api.gamevfs.createSignedURL(req.domain, req.key, function(err, signedURL, getURL){
    ...
    return res.json({signedURL, getURL})
    .end();
}));
...
const addMQlength = function(domain, game, users){
	...
	const id_list = ((() => {
		const result = [];
		for (each of Array.from(users)) { 			result.push(each._id);
		}
		return result;
	})());
	return broker._getBroker(domain).pendingStats(id_list)
	.then(function(res){
		for (let index = 0; index < res.length; index++) { each = res[index]; users[index].mqPending = each; }

		return broker._getBroker(domain).timedoutStats(id_list);}).then(res => (() => {
        const result1 = [];
...
route.get("/game/:game/users/search", function(req, res){
	...
	return xtralife.api.user.search(req.game.appid, req.query.q, parseInt(req.query.skip), parseInt(req.query.limit), (err, count, data) => addMQlength(domain, req.params.game, data).then(() => res.json({list: data, total: count})
    .end()));
});
...
route.get("/game/:game/users", function(req, res, next){
	...
	, function(err, count, data){
		...
		.then(() => res.json({list: data, total: count})
        .end()).catch(next)
		.done();
	});
});
...
route.get("/game/:game/users/find/:user_id", function(req, res, next){
	...
	return xtralife.api.user.list(options
	, function(err, count, data){
		if (err != null) {
			console.log(err);
		}
...
.get((req, res, next) => xtralife.api.outline.get(req.game, req.user_id, [], (err, outline)=> {
...
xtralife.api.connect.exist(req.params.userid, function(err, result){
    ...
    .end();
})).post((req, res) => xtralife.api.user.updateProfile(req.user_id, req.body, function(err, result){
...
.get(function(req, res, next){
	...
	.then(function(data){
		...
		return res.json(data)
		.end();}).catch(next)
	.done();}).post(function(req, res, next){
...
route.delete("/game/:game/user/:userid", (req, res) => xtralife.api.onDeleteUser(req.user_id, (err, data) => res.json(err)
.end()
, req.game.appid));
...
route.post("/game/:game/push/bulk/:domain", function(req, res, next){
	if (req.body.userids == null) { return next(new Error('userids is missing')); } 
...
route.get("/game/:game/matches/:matchid", (req, res, next) => xtralife.api.match.getMatch(req.match_id)
.then(data => res.json(data)).catch(next)
.done());

route.delete("/game/:game/matches/:matchid", (req, res) => xtralife.api.match._forceDeleteMatch(req.match_id, err => res.json({})));

route.put("/game/:game/matches/:matchid", (req, res, next) => xtralife.api.match.updateMatch(req.match_id, req.body)
.then(match => res.json(match)).catch(next)
.done());

// Store / products
route.get("/game/:game/store/products", downloadable('inapp'), (req, res, next) => xtralife.api.store.listProducts(req.game, parseInt(req.query.skip), parseInt(req.query.limit),	function(err, count, products){
    if (err != null) { return next(err); }
    return res.json({total: count, list: products});
}));

route.put("/game/:game/store/products", (req, res, next) => xtralife.api.store.setProducts(req.game, req.body, function(err, count){
    if (err != null) { return next(err); }
    return res.json({total: count});
}));

route.post("/game/:game/store/products", (req, res, next) => xtralife.api.store.addProduct(req.game, req.body, function(err, addedProduct){
    if (err != null) { return next(err); }
    return res.json({product: addedProduct});
}));

route.put("/game/:game/store/products/:productid", (req, res, next) => xtralife.api.store.updateProduct(req.game, req.params.productid, req.body, function(err, modifiedProduct){
    if (err != null) { return next(err); }
    return res.json({product: modifiedProduct});
}));

route.delete("/game/:game/store/products/:productid", (req, res, next) => xtralife.api.store.deleteProduct(req.game, req.params.productid, function(err, result){
    if (err != null) { return next(err); }
    return res.json({ok: 1});
}));
