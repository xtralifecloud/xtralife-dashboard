...
const EditUserStorageCtrl = function ($rootScope, $scope, $q, $routeParams, UserService, notificationService) {
	...
	$scope.ok = () => {
		if (! $scope.storage || $scope.storage.length == 0) {
			if (!window.confirm("The gamer vfs will be emptied.")) {
				notificationService.info('Save cancelled');
				return
			}
		}
		// send all the key-value pairs for update
		return UserService.updateUserStorage(game.name, $scope.userId, $scope.storage, domain).success(() => notificationService.success('key/value saved!')).catch(function (err) {
